...
def cmd_export_assembly(
    assembly_name: str, format: str | None, output_file: Path | None
):
    if format is None:
        format = "step"
    if output_file is None:
        output_file = Path.cwd().joinpath("dist", f"{assembly_name}.{format}")
    ...
    if format == "stl":
        export_stl(assembly, output_file)
    elif format == "step":
        export_step(assembly, output_file)
    else:
